...
- Lexical document surfaces in `src/components/Editor.tsx` / `src/App.css`
...
| Layer | Technology |
|-------|-----------|
| Document editor | [Lexical](https://lexical.dev/) + Yjs collaboration |
...
Document mode is backed by [Yjs](https://github.com/yjs/yjs) CRDTs. The Lexical adapter uses the
same `getCollaborationDoc(docId)` lifecycle as local persistence, and can also accept additional
Yjs providers for remote multi-user sync:
